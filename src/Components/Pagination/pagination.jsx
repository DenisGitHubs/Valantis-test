...
import { setCheckPage, setCurrentPage, setPage} from '../../Store/slices/pageSlice';
export const Pagination = ({ allReceivedIDs, onPageChangeNext, setData }) => {
  ...
  const goToNextPageFilter = () => {
      if(allReceivedIDs.length < 50 * currentPage || allReceivedIDs.length === 50 * (currentPage + 1)) return 
    else { 
      dispatch(setPage(currentPage + 1));
      dispatch(setCheckPage(currentPage + 1));
    ...
    return (
      <div className="pagination-container">
        ...
        <div className="pagination-arrow" onClick={filter === null ? goToNextPage : goToNextPageFilter}>
          &gt;
        </div>
      </div>
    );
